...
final class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneAction = UIContextualAction(style: .normal, title: title) { [unowned self] _, _, isDone in
            ...
            let currentIndex = IndexPath(row: currentTasks.firstIndex(of: task) ?? 0, section: 0)
            let completedIndex = IndexPath(row: completedTasks.firstIndex(of: task) ?? 0, section: 1)
        ...
        alertBuilder
            ...
            .addAction(
                ...
                if let task, let completion {
                    self?.storageManager.editTwoTF(task, newTitle: taskTitle, newNote: taskNote)
                    completion()
                    return
                }
